...
/*                            STM3240G-EVAL.                                  */
/*   08/31/16  D. Horowitz    Changing the usage of the setbaudrate command.  */
/******************************************************************************/
...
#include "PLATVER.h"             /* Bluetopia over STM32 platform Version.    */
   ...
   if(!BluetoothStackID)
   {
      ...
      if((HCI_DriverInformation) && (BTPS_Initialization))
      {
         ...
         if(Result > 0)
         {
            ...
            /* Printing the PLAT version                                */
            Display(("PLAT Version  : %s \r\n", PLATFORM_VERSION_STRING));
...
static int SetBaudRate(ParameterList_t *TempParam)
{
   ...
   if(BluetoothStackID)
   {
      ...
      {
         /* Save the baud rate.                                         */
         BaudRateIndex = (DWord_t)TempParam->Params[0].intParam;
         ...
         if(BaudRate <= MaxBaudRate)
         {
            ...
            else
            {
               ...
               Display(("VS_Update_UART_Baud_Rate(%u): Failure %d.\r\n", BaudRate, ret_val));
         ...
         {
            /* The baud rate is greater than the maximum.               */
            Display(("Error: The specified baud index (%u) - baud rate (%u) is greater\r\nthan the maximum supported by this platform (%u).\r\n", BaudRateIndex, BaudRate, MaxBaudRate));
      ...
      /* When the parameters are not valid print the usage.             */
      if ( ret_val == INVALID_PARAMETERS_ERROR )
      {
         DisplayUsage("SetBaudRate [BaudRate Index]");
         Display(("Which Baud Rate index can be\r\n"));
         /* Print each option available until the max baud rate.        */
         for (BaudRateIndex = 0, BaudRate = 115200; BaudRate <= MaxBaudRate;)
         {
             Display (("Index (%u): baud rate (%u)\r\n",BaudRateIndex,BaudRate));
             /* Increase the index number.                              */
             BaudRateIndex++;
             /* Each baud rate is multiple by 2 of the previous         */
             /* baud rate.                                              */             
             BaudRate = BaudRate * 2;
         }
      }
...
static int OpenRemoteEndpoint(ParameterList_t *TempParam)
{
   ...
   if((BluetoothStackID) && (TempParam) && (TempParam->NumberofParameters > 0) && 
      (TempParam->Params[0].intParam > 0) && (TempParam->Params[0].intParam <= NumberofValidResponses))
   {
